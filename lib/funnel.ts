...
export const FUNNEL_STEPS = ["signin_start", "signin_new", "signin_returning", "signin_error", "stale_session", "first_dashboard", "cli_banner_copy", "cli_linked", "invite_copy", "invite_join", "mcp_call", "vs_create", "vs_view", "props_give", "gpc_skipped"] as const;
export type FunnelStep = (typeof FUNNEL_STEPS)[number] | `signin_error:${string}` | `signin_start:${string}` | `signin_new:${string}`;

/**
 * One more of `step` today (UTC). Never throws: a counter must not break the sign-in it counts.
 * Only production counts: local dev and preview deploys share the production database.
 */
