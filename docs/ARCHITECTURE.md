...
| `/dashboard/u/[login]` | owner / allowed | tiles, the owner's weekly goal ring (`GoalRing`, owner only) and one-time record banners, records (`RecordsPanel`: best Monday-to-Sunday week and calendar month by lines and by commits, longest streak ever, from `userRecords`, gated like the tiles), `WeeklyBars`, `YearCalendar`, day calendar, lines-per-day trend, language share, weekday profile, repo mix + share, repos table (names masked per settings) |
