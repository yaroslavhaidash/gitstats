...
| `/dashboard/u/[login]` | owner / allowed | tiles, `WeeklyBars`, `YearCalendar`, day calendar, lines-per-day trend, language share, weekday profile, repo mix + share, repos table (names masked per settings) |
...
| `/dashboard/settings` | session | linked machines, profile visibility, tokens, data export + account deletion |
