...
| Table | Key | Purpose |
|---|---|---|
| `users` | `id` | GitHub identity + the visibility matrix: `profile_visibility` (crew\|everyone), `repo_names`/`repo_names_global` (all\|public_only\|none), `share_private`/`share_private_global` (crewmates column, then everyone), plus `hash_salt`, `github_id`, `last_snapshot_at` (the snapshot queue's order), `share_nonce` (mixed into every share-card signature; rotating it is the revoke), `last_streak_milestone` (the highest of 7/30/100/365 whose one-time banner the member has seen), `last_record_week`/`last_record_month`/`last_record_streak` (the Monday, month and run start of the last "new record" banner shown), `is_demo` (the seeded `/demo` crew) |
